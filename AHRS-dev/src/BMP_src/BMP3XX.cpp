...
I2CDevice* g_i2c_dev = NULL; ///< Global I2C interface pointer
...
bool BMP3XX::begin_I2C(uint8_t addr, TwoWire* theWire) {
    ...
    g_i2c_dev = i2c_dev = new I2CDevice(addr, theWire);
...
bool BMP3XX::_init(void) {
    ...
    the_sensor.settings.odr_filter.press_os = BMP3_OVERSAMPLING_4X;
    the_sensor.settings.odr_filter.temp_os = BMP3_OVERSAMPLING_4X;
    the_sensor.settings.odr_filter.odr = BMP3_ODR_50_HZ;

    // don't do anything till we request a reading

    uint16_t settings_sel = BMP3_SEL_PRESS_EN | BMP3_SEL_TEMP_EN | BMP3_SEL_PRESS_OS | BMP3_SEL_TEMP_OS | BMP3_SEL_ODR | BMP3_SEL_DRDY_EN;

    rslt = bmp3_set_sensor_settings(settings_sel, &the_sensor.settings, &the_sensor);
    the_sensor.settings.op_mode = BMP3_MODE_NORMAL;
    rslt = bmp3_set_op_mode(&the_sensor.settings, &the_sensor);
...
*/
/**************************************************************************/
uint8_t BMP3XX::chipID(void) { return the_sensor.chip_id; }
